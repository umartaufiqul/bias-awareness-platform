...
import os
# from ClassifierFunction import tokenize
...
def get_tweets_predictions(tweets, perform_prints=True):
    ...
    model = joblib.load(os.path.join(curr_path, 'final_model.pkl'))

    print "Loading other information..."
    tf_vectorizer = joblib.load(os.path.join(curr_path, 'final_tfidf.pkl'))
    idf_vector = joblib.load(os.path.join(curr_path, 'final_idf.pkl'))
    pos_vectorizer = joblib.load(os.path.join(curr_path, 'final_pos.pkl'))
    #Load ngram dict
    #Load pos dictionary
...
def classify_tweet():
    # sys.path.append('C:\\Users\\Umar Taufiqulhakim\\Documents\\KAIST\\Summer Intern 2020\\bias-awareness-platform\\api')
    print(sys.modules.get(tokenize))
    print "Loading data to classify..."

    #Tweets obtained here: https://github.com/sashaperigo/Trump-Tweets

    dataset_path = os.path.join(curr_path, 'trump_tweets.csv')
    ...
    for i,t in enumerate(trump_tweets):
        ...
        result[i] = class_to_name(trump_predictions[i])
